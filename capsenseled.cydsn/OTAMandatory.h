...
typedef enum
{
    ...
    /* This event is generated when the function CyBle_GapResolveDevice is called to resolve a private address.
       On successful procedure completion, 'evParam' will contain the Resolvable Private Address verified. 

       In the application context, this procedure should be used on reception of ADV packets in a central/scanner
       device from a peer device using Resolvable Private Address and the application is trying to connect 
       to a previously bonded device.
       
       The event parameter points to a uit8 data type. Size of the data that has to be read is 6 bytes. */
    ...
    /* 'MTU Exchange Request' received from GATT client device. Event parameter 
       contains the MTU size of type uint16. */
    ...
    /* This event indicates the connection parameter update request has been
       rejected. Event parameter is a pointer to a structure of type
       CYBLE_CONN_UPDATE_PARAM_REJ_REASON_T. */
    CYBLE_EVT_L2CAP_CONN_PARAM_UPDATE_CMD_REJ,
...
typedef struct
{
    ...
    CYBLE_CUSTOMS_INFO_T customServiceInfo[1];
} CYBLE_CUSTOMS_T;
